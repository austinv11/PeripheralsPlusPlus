...
public class LuaObjectPlayerInv implements ILuaObject {
	...
	private TilePlayerInterface playerInterface;
	private ItemStack permCard;

	public LuaObjectPlayerInv(EntityPlayer player, TilePlayerInterface playerInterface, ItemStack permCard) {
	...
	private Object getObjectFromStack(ItemStack stack) {
		...
		String itemName = stack.getUnlocalizedName();
	...
	private IInventory getOutputInventory() throws LuaException {
		...
		BlockPos blockLoc = new BlockPos(playerInterface.getPos().getX() + outDir.getFrontOffsetX(), playerInterface.getPos().getY() + outDir.getFrontOffsetY(), playerInterface.getPos().getZ() + outDir.getFrontOffsetZ());
		Block block = playerInterface.getWorld().getBlockState(blockLoc).getBlock();
		if (block instanceof BlockContainer) {
			return (IInventory) playerInterface.getWorld().getTileEntity(blockLoc);
		} else {
	...
	private IInventory getInputInventory() throws LuaException {
		...
		BlockPos blockLoc = new BlockPos(playerInterface.getPos().getX() + inDir.getFrontOffsetX(), playerInterface.getPos().getY() + inDir.getFrontOffsetY(), playerInterface.getPos().getZ() + inDir.getFrontOffsetZ());
		Block block = playerInterface.getWorld().getBlockState(blockLoc).getBlock();
		if (block instanceof BlockContainer) {
			return (IInventory) playerInterface.getWorld().getTileEntity(blockLoc);
		} else {
